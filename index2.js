...
async function getFactoryContracts(Apeswap_FactoryAbi, Apeswap_FactoryAddress, pancake_FactoryAbi, Pancake_FactoryAddress)
{
  ...
  const getContractParams = [{"Exchange": "Apeswap", "ExchangeAbi": Apeswap_FactoryAbi, "ExchangeAddress": Apeswap_FactoryAddress},
                             {"Exchange": "Pancakeswap", "ExchangeAbi": pancake_FactoryAbi, "ExchangeAddress": Pancake_FactoryAddress}]
...
getFactoryContracts(ApeswapFactoryAbi, ApeswapFactoryAddress, pancakeFactoryAbi, PancakeFactoryAddress).then(async (theFactories) => {
  ...
  let finalPrices = []
   ...
   let thePrices = Promise.all(reserves).then(theReserves => {
      for(var i=0; i<theReserves.length; i++)
      {
        ...
        finalPrices.push({Exchange, reserve1, reserve0, token1, token0, Token0Address, Token1Address, Pair, token1Price})
      }
...
.then(thePrices => {
    ...
    for(var i=0; i<FinalPrices.length; i++)
	  {
        ...
        for(var x=0; x<mockPriceArray.length; x++)
        {
          if(theExchange != mockPriceArray[x].Exchange && Token0 ==  mockPriceArray[x].token0 && Token1 ==  mockPriceArray[x].token1  && Token0Address ==  mockPriceArray[x].Token0Address && Token1Address ==  mockPriceArray[x].Token1Address)
          {
            ...
            let ExchangePrice2 = mockPriceArray[x].token1Price

            if(PriceDiff > 0)
            {
                PriceDiffs.push({thePair, "BuyFrom": mockPriceArray[x].Exchange, "SellTo": theExchange, Token0Address, Token1Address, "Borrow_Amount":`${ExchangePrice2}` , "Amount_Out":`${ExchangePrice1}`, PriceDiff, date})
            }
            // else
            //   { 
  ...
  Promise.all(thePriceDiffs).then(async (priceDifferences) => {
    ...
    let v2LibraryContract = new Web3.eth.Contract(v2Router, v2RouterAddress) 



      //  let FlashloanRepayment = await v2LibraryContract.methods.getAmountIn().call()

    for(var i=0; i<priceDifferences.length; i++)
    {

      //getAmountsIn to sell in exch for higher amount
      //-> amount to borrow 

      //diff = higher price - getAmountsIn



      let PriceDiffRatio = (priceDifferences[i].PriceDiff / priceDifferences[i].Amount_Out) * 100
      ...
      if(netReturn > 0)
      {
        priceDifferences[i].Return = `Possibly profitable with a ${netReturn}% return` 
      }
      else
        {
           priceDifferences[i].Return = `Not profitable with a ${netReturn}% return` 
        }
